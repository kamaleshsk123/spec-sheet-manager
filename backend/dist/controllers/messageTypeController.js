...
class MessageTypeController {
    static async getMessageTypes(req, res) {
        try {
            ...
            const result = await database_1.default.query('SELECT * FROM message_types WHERE spec_id = $1 ORDER BY created_at ASC', [specId]);
    ...
    static async createMessageType(req, res) {
        try {
            const { specId } = req.params;
            ...
            const result = await database_1.default.query('INSERT INTO message_types (spec_id, name, payload_definition, json_schema) VALUES ($1, $2, $3, $4) RETURNING *', [specId, name, payload_definition, json_schema]);
    ...
    static async updateMessageType(req, res) {
        try {
            const { id } = req.params;
            const { name, payload_definition, json_schema } = req.body;
            const userId = req.user.id;
            const messageTypeResult = await database_1.default.query('SELECT spec_id FROM message_types WHERE id = $1', [id]);
            ...
            const result = await database_1.default.query('UPDATE message_types SET name = $1, payload_definition = $2, json_schema = $3, updated_at = NOW() WHERE id = $4 RETURNING *', [name, payload_definition, json_schema, id]);
    ...
    static async deleteMessageType(req, res) {
        try {
            const { id } = req.params;
            const userId = req.user.id;
            const messageTypeResult = await database_1.default.query('SELECT spec_id FROM message_types WHERE id = $1', [id]);
            ...
            const result = await database_1.default.query('DELETE FROM message_types WHERE id = $1 RETURNING id', [id]);
            if (result.rowCount === 0) {
                return res.status(404).json({ success: false, error: 'Message type not found' });
            }
...
//# sourceMappingURL=messageTypeController.js.map