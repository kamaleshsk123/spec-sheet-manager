...
class CompareCommand {
    ...
    getCommand() {
        const cmd = new commander_1.Command('compare');
    ...
    async compareVersions(spec1Id, spec2Id, options) {
        ...
        try {
            ...
            if (!response1.success || !response1.data) {
                spinner.fail(chalk_1.default.red('Failed to fetch first specification: ' + response1.error));
                return;
            }
            if (!response2.success || !response2.data) {
                spinner.fail(chalk_1.default.red('Failed to fetch second specification: ' + response2.error));
                return;
            }
            const spec1 = response1.data;
            const spec2 = response2.data;
            spinner.succeed(chalk_1.default.green(`Comparing: ${spec1.title} v${spec1.version} vs ${spec2.title} v${spec2.version}`));
    ...
    printColoredDiff(diff, spec1, spec2, useColor) {
        console.log(chalk_1.default.bold(`\n--- ${spec1.title} v${spec1.version}`));
        console.log(chalk_1.default.bold(`+++ ${spec2.title} v${spec2.version}`));
        console.log(chalk_1.default.gray(`@@ Changes: +${diff.stats.added} -${diff.stats.removed} @@\n`));
        ...
        for (let i = 0; i < maxLines; i++) {
            ...
            if (leftLine?.type === 'removed') {
                ...
                console.log(useColor ? chalk_1.default.red(`- ${line}`) : `- ${line}`);
            }
            else if (rightLine?.type === 'added') {
                const line = `${rightLine.lineNumber.toString().padStart(4)} | ${rightLine.content}`;
                console.log(useColor ? chalk_1.default.green(`+ ${line}`) : `+ ${line}`);
            }
            else if (leftLine?.type === 'unchanged') {
                const line = `${leftLine.lineNumber.toString().padStart(4)} | ${leftLine.content}`;
                console.log(useColor ? chalk_1.default.gray(`  ${line}`) : `  ${line}`);
            }
        }
        console.log(chalk_1.default.bold(`\nSummary:`));
        console.log(chalk_1.default.green(`  Added lines: ${diff.stats.added}`));
        console.log(chalk_1.default.red(`  Removed lines: ${diff.stats.removed}`));
        console.log(chalk_1.default.gray(`  Unchanged lines: ${diff.stats.unchanged}`));
    }
    generateHtmlDiff(diff, spec1, spec2) {
        let html = `
<!DOCTYPE html>
<html>
<head>
...
`;
    ...
    async detectBreakingChanges(spec1Id, spec2Id, options) {
        console.log(chalk_1.default.yellow('Breaking change detection coming soon...'));
